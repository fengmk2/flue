...
const dispatchQueue = {
  async enqueue(input) {
    ...
    const response = await fetchCloudflareDurableObject(binding, input.id, new Request('https://flue.invalid' + INTERNAL_DISPATCH_PATH, {
      method: 'POST',
...
configureFlueRuntime({
  ...
  routeAgentRequest: (request, reqEnv, target) => {
    const binding = reqEnv?.[agentIdentities[target.agentName]?.bindingName];
    return binding ? fetchCloudflareDurableObject(binding, target.instanceId, request) : null;
  },
  routeWorkflowRequest: (request, reqEnv, target) => {
    const binding = reqEnv?.[workflowIdentities[target.workflowName]?.bindingName];
    return binding ? fetchCloudflareDurableObject(binding, target.instanceId, request) : null;
  },
  createRunRegistryForRequest,
  routeRunRequest: (request, reqEnv, target) => {
    if (target.kind !== 'workflow') return null;
    const binding = reqEnv?.[workflowIdentities[target.workflowName]?.bindingName];
    if (!binding) return null;
    const retry = new URL(request.url).pathname.endsWith('/stream') ? undefined : true;
    return fetchCloudflareDurableObject(binding, target.instanceId, request, { retry });
  },
});
...
function validateCloudflareAgentNames(ctx: BuildContext): void {
	...
	// converters. Validate together with a shared message.
